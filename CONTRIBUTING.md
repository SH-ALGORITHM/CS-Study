...
| 토 ~ 일 | 멤버: scenario.md 읽기 + 도메인 선택 + s1 시도 (집에서) |
| 월요일 모임 직전 | 본인 폴더에서 s1 코드 → **Draft PR** (브랜치 `{이름}/w{NN}`) |
| **월요일 모임 (2h)** | 각자 Draft PR 띄우고 시도 발표 (**머지 X**) |
| 화 ~ 수 | 같은 브랜치에 s2 (visibility) + s3 (측정) 커밋 추가 |
...
|---|---|---|---|
| 1. 브랜치 만들고 s1 코드 → **Draft PR 만듦** | Draft | ❌ 버튼 회색 | ✅ 가능 |
| 2. 월 모임 발표 (화면 띄움) | Draft 그대로 | ❌ | ✅ |
| 3. 화~수 같은 브랜치에 s2, s3 푸시 | Draft 그대로 (자동 누적) | ❌ | ✅ |
...
- [ ] 월요일 전 — 도메인 1개 골라 s1 코드 작성 → Draft PR
- [ ] 월요일 모임 — 발표
- [ ] 수요일까지 — s2 추가
...
#### 측정 코드 템플릿 — 복사해서 본인 도메인으로 변환

본인 도메인 클래스(`CouponEvent` 등)와 본인 폴더에 아래 코드 복사 후 클래스 이름만 바꾸면 됩니다:
...
public class Stage3Demo {
    // 측정 파라미터 (본인이 조정)
    static final int THREADS  = 200;     // 동시 스레드 수
    static final int ATTEMPTS = 10_000;  // 시도 횟수
    static final int RUNS     = 5;       // 측정 반복 (평균 내기)
    static final int WARMUP   = 5_000;   // JIT 워밍업
    static final int INITIAL  = 100;     // 초기 잔고/쿠폰 수

    public static void main(String[] args) throws Exception {
        // === 1) 워밍업 (결과 안 봄) ===
        for (int i = 0; i < WARMUP; i++) {
            new CouponEvent(INITIAL).claim();   // ★ 본인 도메인 클래스로 변경
        }
        ...
        for (int run = 0; run < RUNS; run++) {
            CouponEvent event = new CouponEvent(INITIAL);   // ★ 변경
            ExecutorService executor = Executors.newFixedThreadPool(THREADS);
            AtomicInteger success = new AtomicInteger(0);
            CountDownLatch startSignal = new CountDownLatch(1);

            // 모든 스레드 대기 상태로 등록 (CountDownLatch로 동시 출발)
            for (int i = 0; i < ATTEMPTS; i++) {
                executor.submit(() -> {
                    try { startSignal.await(); } catch (InterruptedException e) { return; }
                    if (event.claim()) {                    // ★ 본인 메서드로 변경
                        success.incrementAndGet();
                    }
                });
            }
            Thread.sleep(50);                               // 모든 스레드가 latch 도달 대기
            ...
            // === 4) 누락 = |기대(INITIAL) - 실제 성공| ===
            totalMisses += Math.abs(success.get() - INITIAL);
            totalMs     += elapsedMs;
        }
        ...
        // === 5) 결과 출력 + 자동 저장 ===
        System.out.printf("평균 누락: %.1f건 / 평균 응답: %.1fms (5회 평균)%n", avgMisses, avgMs);
        MeasurementLog.save("s3", "쿠폰 단순 if/decrement", avgMisses, avgMs);
    }
}
```

본인 도메인으로 변환할 부분 (★ 표시 3곳):
- `new CouponEvent(INITIAL)` → 본인 도메인 클래스 (`StockManager`, `Attendance` 등)
- `event.claim()` → 본인 메서드 (`purchase()`, `checkIn()` 등)
- `MeasurementLog.save("s3", "쿠폰 단순 if/decrement", ...)` 의 두 번째 인자 → 본인 도메인 + 방식명

#### 해결책 비교 — 같은 패턴 3번 (synchronized / Atomic / 없음)

`SyncCouponEvent` (synchronized 적용), `AtomicCouponEvent` (AtomicInteger 적용) 같은 변형 클래스 만들고 위 측정 코드 3번 돌리면 됨. 매번 `MeasurementLog.save()`로 다른 방식명 기록 → measurements.md에 자동 누적.

전체 작동 코드는 example의 `Stage3Measurement.java` 참고 (3가지 해결책 비교 패턴 그대로).
