...
> 시나리오는 **목요일 모임 끝나면 다음 주차 공개**됩니다. (목 모임 회고 → 운영자가 다듬음 → 금요일 안에 공개)

| 시점 | 무엇 |
|---|---|
| **목요일 모임 끝** | 운영자: 발표/회고 듣고 다음 주 시나리오 다듬음 / 그날부터 다음 주차 시작 |
| **금요일** | 다음 주 `scenario.md` 작성 완료 → 디스코드 공지 |
| 금 ~ 일 | 멤버: 시나리오 읽기 + 그 주 필요 개념 정리 (OS 레벨까지) + **겪기 단계** 체험 (예: race 손으로 / DBeaver 두 세션) |
| **월 11:00** | 개념 정리 + 겪기 결과 → **Draft PR 마감** (브랜치 `{이름}/w{NN}`) |
| **월요일 모임 (2h)** | (전반) 운영자가 2~3 명 뽑아 개념/이론 발표 + 상호 질문 → (후반) 7 명 전원이 각자 본인 "겪기" 결과 짧게 발표 (**머지 X**) |
| 화 ~ 수 | 같은 브랜치에 **코드 단계** 커밋 추가 (자동화 + 측정 + 해결책 비교) |
| **목 11:00** | 코드 + 측정 + 면접 질문 정리 → **Ready for review** 전환 |
| **목요일 모임 (2h)** | 각자 코드 / 측정 / 해석 발표 → 면접 질문 토론 → 운영자가 머지 / 다음 주차 시작 |
| 금 ~ 일 | (자유) 학습 기록 정리 — 블로그 / 노션 / 메모 |

### "겪기 단계" vs "코드 단계"

각 주차의 STAGE 번호 (s1, s2, s3, s4) 는 시나리오 내부 단계일 뿐, **마감 단위는 두 단계로**:

| 단계 | 마감 | 무엇 |
|---|---|---|
| **겪기 단계** | 월 11:00 | 개념 정리 + 손으로 체험 (예: 1주차 = race 직접 봄, 2주차 = DBeaver 두 세션 이상 현상) |
| **코드 단계** | 목 11:00 | 자동화 + 측정 + 해결책 적용 + 면접 질문 정리 |

> 월 모임은 두 부분으로 진행:
> - **전반**: 운영자가 그 자리에서 2~3 명 뽑아 개념/이론 발표. 본인이 안 뽑혀도 자료는 다 준비할 것 (다음 주 / 그 다음 주에 뽑힐 수 있음). 발표 중 누구나 질문 가능, 답하면서 본인 이해 검증.
> - **후반**: 7 명 전원이 본인 "겪기" 결과 짧게 발표. 손으로 본 race / 이상 현상 / 측정 노트 등.
...
|---|---|---|---|
| 1. 브랜치 만들고 개념 정리 + 겪기 단계 결과 → **Draft PR 만듦** | Draft | ❌ 버튼 회색 | ✅ 가능 |
| 2. 월 11:00 까지 Draft 마감 → 월 모임 (전반: 2~3 명 이론 발표 / 후반: 전원 겪기 발표) | Draft 그대로 | ❌ | ✅ |
| 3. 화~수 같은 브랜치에 코드 단계 커밋 | Draft 그대로 (자동 누적) | ❌ | ✅ |
| 4. 목 11:00 — **"Ready for review"로 전환** | Ready | ✅ 버튼 활성화 | ✅ + 자동 알림 |
| 5. 목 모임 발표 (코드 + 측정 + 면접 질문) | Ready | ✅ | ✅ |
| 6. 운영자가 머지 | Merged | — | — |
| 7. 브랜치 삭제 | (PR 닫힘) | — | — |
...
- [ ] 금 ~ 일 — `topics/{이번주차}/scenario.md` 읽기 + 그 주 필수 개념 정리 (OS 레벨까지) + 겪기 단계 체험
- [ ] **월 11:00 까지** — 도메인 1 개 + 개념 정리 + 손으로 체험 결과 → **Draft PR 마감**
- [ ] 월요일 모임 — (전반) 발표 자료 띄우고 대기, 운영자가 2~3 명 뽑아 발표 / 안 뽑혀도 질문·답변 참여 → (후반) 본인 "겪기" 결과 짧게 발표
- [ ] 화 ~ 수 — 같은 브랜치에 코드 / 측정 / 면접 질문 정리 커밋
- [ ] **목 11:00 까지** — 코드 + 측정 완료 → **Ready for review** 전환
- [ ] 목요일 모임 — 발표 + 면접 질문 토론 + 머지
- [ ] (자유) 학습 기록 정리 — 블로그 / 노션 / 메모
...
| 코드 직접 짜달라 | "쿠폰 코드 짜줘" | "시도했어?" 거부 ❌ |

### 좋은 질문 예시

```
race 재현 코드를 짰는데 안 보여요. 코드:
[코드 첨부]
...
1. 시나리오 다시 정독 (놓친 가이드 있나?)
2. 검색 (블로그, 공식 문서 — 시나리오 금지 키워드 제외)
3. 본인 코드에서 가설 세우고 변경 시도
4. 변경 결과 측정/관찰

→ 이 동안 한 것을 AI에 같이 보여주면 좋은 질문이 됨.
