...
| 금 ~ 토 오전 | 멤버: scenario.md 읽기 + 도메인 선택 + s1 (race) + s2 (visibility) 시도 |
| **토 14:00** | s1 + s2 코드 → **Draft PR 마감** (브랜치 `{이름}/w{NN}`) |
| 토 오후 ~ 일 | 운영자 / 멤버 PR 코멘트 (모임 전 미리 리뷰) |
...
- [ ] 토요일 14:00까지 — 도메인 1개 골라 s1 (race) + s2 (visibility) 코드 작성 → **Draft PR 마감**
- [ ] 월요일 모임 — 시도 발표
