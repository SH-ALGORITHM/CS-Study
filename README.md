...
주제 성격에 따라 형식을 바꿔 가며 3부로 진행 중이다.
...
| 부 | 기간 | 인원 | 형식 |
|---|---|---|---|
| **1부** 도메인 기반 CS | 2026-05-01 ~ 06-15 (6주) | 7명 | 주차별 시나리오 → 각자 다른 도메인으로 재현 · 측정 → PR 리뷰 |
| **2부** 이력서 · 포트폴리오 면접 | 2026-06-16 ~ 07-19 (5주) | 5명 | 각자 이력서/포트폴리오를 서로 파고드는 상호 질문 (동시성 위주) |
| **3부** Spring MVC | 2026-07-20 ~ 진행 중 | 5명 | 김영한 강의 완주 + 각자 정리 → 정리본 공유 |

## 1부 — 멤버별 주차 도메인 (제출 코드 기준)
...
시나리오는 12주차분까지 작성해 뒀다. 7주차 이후([`topics/07-jpa-n-plus-1`](./topics/07-jpa-n-plus-1) ~ [`topics/12-observability`](./topics/12-observability))
는 시나리오와 예제 코드만 있고 멤버 제출물은 없다.

## 2부 — 이력서 · 포트폴리오 면접

각자 이력서와 포트폴리오를 놓고 서로 질문하는 형식으로 5주간 진행했다.
1부에서 다룬 동시성 주제가 각자 프로젝트 어디에 실제로 걸려 있는지를 주로 파고들었다.

구두로만 진행해서 저장소에 남은 기록은 없다.
