...
| 부 | 기간 | 인원 | 형식 | 남는 것 |
|---|---|---|---|---|
| **1부** 도메인 기반 CS | 2026-05-01 ~ 06-15 (6주) | 7명 | 주차별 시나리오 → 각자 다른 도메인으로 재현 · 측정 → PR 리뷰 | [`topics/`](./topics) 코드 + `measurements.md` |
| **2부** 이력서 · 포트폴리오 면접 | 2026-06-16 ~ 07-19 (5주) | 5명 | 각자 이력서/포트폴리오를 서로 파고드는 상호 질문 (동시성 위주) | 구두 진행 — 별도 산출물 없음 |
| **3부** Spring MVC | 2026-07-20 ~ 진행 중 | 5명 | 김영한 강의 완주 + 각자 정리 → 정리본 공유 | 개인 블로그 / 노션 링크 (아래) |

1부는 "시나리오를 주면 각자 자기 도메인으로 옮겨 푼다"는 형식이었다.
같은 주제를 서로 다른 도메인으로 풀어 보니 PR 리뷰에서 비교가 되는 게 이 형식의 이점이었고,
반대로 주차마다 새 프로젝트를 세팅하는 부담이 커서 6주 뒤 형식을 바꿨다.

## 1부 — 멤버별 주차 도메인 (제출 코드 기준)

| 이름 | 폴더 | 1주차<br>JVM · 스레드 | 2주차<br>트랜잭션 | 3주차<br>락 | 4주차<br>IoC · Bean | 5주차<br>Proxy · AOP | 6주차<br>Spring Event |
|---|---|---|---|---|---|---|---|
| 오찬혁 | `chanhyeok` | 선착순 쿠폰 | 호텔 객실 예약 | P2P 송금 | 결제 PG 연동 | 분산락 AOP | P2P 송금 |
| 김가빈 | `gabin` | 이중 환불 | 장바구니 결제 | 장바구니 결제 | 할인 정책 | 캐싱 | 주문 완료 |
| 김민서 | `minseo` | 출퇴근 기록 | 스터디룸 예약 | 좌석 예매 | 인증 전략 | 감사 로그 | |
| 박수진 | `sujin` | 배치 처리 | | 주식 매수 / 매도 | 알림 발송 | 권한 검증 | |
| 강희민 | `heemin` | 재고 관리 | | | | | 재고 변경 |
| 한재훈 | `jaehoon` | 조회수 / 좋아요 | 환전 / 통화 거래 | | | | |
| 이가은 | `gaeun` | 콘서트 좌석 | | | | | |
