...
public class Stage4_1_AopToEvent {
    ...
    public record OrderPlacedEvent(Long orderId, BigDecimal amount, String action) {}
    ...
    public static class NewEventService {
        ...
        public void placeOrder(Long orderId, BigDecimal amount) {
            jdbc.update("INSERT INTO orders(id, amount) VALUES (?, ?)", orderId, amount);
            publisher.publishEvent(new OrderPlacedEvent(orderId, amount, "ORDER_PLACED"));
            if (amount.compareTo(BigDecimal.ZERO) < 0) {
                throw new IllegalArgumentException("음수 금액");
            }
        }
