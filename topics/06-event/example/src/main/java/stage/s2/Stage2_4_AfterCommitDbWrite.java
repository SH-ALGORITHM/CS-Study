...
public class Stage2_4_AfterCommitDbWrite {
    ...
    public static class NoTxListener {
        ...
        public void on(OrderPlacedEvent e) {
            System.out.println("    [NoTx] INSERT (no @Transactional) id=" + (e.orderId() + 1000));
            try {
                jdbc.update("INSERT INTO orders(id, amount) VALUES (?, ?)",
                    e.orderId() + 1000, e.amount());
    ...
    public static class RequiresNewListener {
        ...
        public void on(OrderPlacedEvent e) {
            System.out.println("    [ReqNew] INSERT (REQUIRES_NEW) id=" + (e.orderId() + 2000));
            jdbc.update("INSERT INTO orders(id, amount) VALUES (?, ?)",
                e.orderId() + 2000, e.amount());
    ...
    public static void main(String[] args) {
        ...
        System.out.println("  시작 카운트: " + svc.countOrders());

        MeasurementLog.section("placeOrder(1, 100) — publisher INSERT + 두 리스너 호출");
        svc.placeOrder(1L, BigDecimal.valueOf(100));

        System.out.println();
        System.out.println("  최종 카운트: " + svc.countOrders() + "  ← 직접 확인. ReqNew 의 INSERT 는 반드시 반영");
        System.out.println("    · NoTx 의 INSERT 는 환경 (H2/Spring 버전 / 동기화 상태) 따라 갈림");
