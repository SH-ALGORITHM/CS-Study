...
public class Stage2_2_AllPhases {
    ...
    public static class AllPhaseListener {
        ...
        public void onBefore(OrderPlacedEvent e) {
            System.out.println("    [BEFORE_COMMIT] id=" + e.orderId());
        }

        @TransactionalEventListener(phase = TransactionPhase.AFTER_COMMIT)
        public void onAfterCommit(OrderPlacedEvent e) {
            System.out.println("    [AFTER_COMMIT]  id=" + e.orderId());
        }

        @TransactionalEventListener(phase = TransactionPhase.AFTER_ROLLBACK)
        public void onAfterRollback(OrderPlacedEvent e) {
            System.out.println("    [AFTER_ROLLBACK] id=" + e.orderId());
        }

        @TransactionalEventListener(phase = TransactionPhase.AFTER_COMPLETION)
        public void onAfterCompletion(OrderPlacedEvent e) {
            System.out.println("    [AFTER_COMPLETION] id=" + e.orderId());
        }
    }
    ...
    public static void main(String[] args) {
        ...
        svc.placeOrder(10L, BigDecimal.valueOf(100));

        MeasurementLog.section("rollback — amount=-100");
        try { svc.placeOrder(11L, BigDecimal.valueOf(-100)); }
