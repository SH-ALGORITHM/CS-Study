...
### [목 모임 24h 전 — PR Ready 전환 (수 23:59)]

#### ▸ STAGE 3 — 측정 + 해결 (필수)
**※ 이 단계부터 금지 키워드 해제** — `synchronized` / `Atomic*` / `volatile` 사용 가능.
단 "30분 직접 시도 후 적용" 룰 유지 (생각 → 가설 → 시도 → 결과 보고 → 적용).
...
**STAGE 3 시작과 함께 해제.** 단 30분 직접 시도 → 가설 → 측정 → 적용 순서 유지.
