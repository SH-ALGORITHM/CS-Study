...
public class Stage4VirtualThreads {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int run = 0; run < retryCnt; run++) {
            for (int threadCnt : threadCntArr) {
                int seats = (threadCnt * requestsPerThread) / 2;
                measure("Virtual-Unsafe",       new UnsafeConcertBooking(seats),       threadCnt, seats);
                measure("Virtual-Synchronized", new SynchronizedConcertBooking(seats), threadCnt, seats);
                measure("Virtual-Atomic",       new AtomicConcertBooking(seats),       threadCnt, seats);
            }
        }
    ...
    private static void measure(String label, ConcertBooking booking, int threadCnt, int seats) throws Exception {
        ...
        // ▲▲▲ 풀 크기 지정 X — Virtual Thread 는 매 작업마다 새로 생성, 끝나면 버림 ▲▲▲
    ...
    private static void printSummary() {
        System.out.printf("%-22s %5s | %10s %10s %10s | %10s%n",
        ...
        for (var entry : result.entrySet()) {
            ...
            System.out.printf("%-22s %5d | %10.2f %10.2f %10.2f | %10.2f%n",
